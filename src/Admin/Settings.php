...
class Settings {
     ...
     */
    function set_sections( array $sections ): Settings {
        $this->settings_sections = array_map( function ( $section ) {
            if ( isset( $section['id'] ) ) {
                $section['id'] = $this->prefix . $section['id'];
            }
     ...
     * @return Settings
     */
    function add_section( array $section ): Settings {
        if ( isset( $section['id'] ) ) {
            $section['id'] = $this->prefix . $section['id'];
        }
     ...
     */
    function set_fields( array $fields ): Settings {
        foreach ( $fields as $section_id => $value ) {
            $this->settings_fields[ $this->prefix . $section_id ] = $value;
        }

        return $this;
    }

    function add_field( $section, $field ): Settings {
        $defaults = [
            'name'  => '',
            'label' => '',
            'desc'  => '',
        ...
        $this->settings_fields[ $this->prefix . $section ][] = $arg;
    ...
    function admin_init() {
        ...
        foreach ( $this->settings_fields as $section => $field ) {
            foreach ( $field as $option ) {
                ...
                add_settings_field( $option['id'], $label, $callback, $section, $section, $args );
            }
        }
    ...
    function callback_text( array $args ) {
        ...
        $value       = esc_attr( $this->get( $args['id'], $args['section'], $args['std'] ) );
    ...
    function callback_multicheck( array $args ) {
        ...
        $value = $this->get( $args['id'], $args['section'], $args['std'] );
        $html  = '<fieldset>';
    ...
    function callback_textarea( $args ) {
        ...
        $value       = esc_textarea( $this->get( $args['id'], $args['section'], $args['std'] ) );
        $size        = isset( $args['size'] ) && ! is_null( $args['size'] ) ? $args['size'] : 'regular';
        $placeholder = empty( $args['placeholder'] ) ? '' : ' placeholder="' . $args['placeholder'] . '"';
    ...
    function callback_file( $args ) {
        ...
        $html = sprintf( '<input type="text" class="%1$s-text MFileInput" id="%2$s[%3$s]" name="%2$s[%3$s]" value="%4$s"/>', $size, $args['section'], $args['id'], $value );
        $html .= '<input type="button" class="button MFileInputBrowse" value="' . $label . '" />';
        $html .= $this->get_field_description( $args );
    ...
    function callback_color( $args ) {
        ...
        $html = sprintf( '<input type="text" class="%1$s-text MColorPickerInput" id="%2$s[%3$s]" name="%2$s[%3$s]" value="%4$s" data-default-color="%5$s" />', $size, $args['section'], $args['id'], $value, $args['std'] );
        $html .= $this->get_field_description( $args );
    ...
    function callback_pages( $args ) {
        ...
        $dropdown_args = array(
            'selected' => esc_attr( $this->get( $args['id'], $args['section'], $args['std'] ) ),
     ...
     * @return mixed
     */
    function get( string $option, string $section, string $default = '' ) {
    ...
    function show_forms() {
        ...
        <div class="metabox-holder">
            <?php foreach ( $this->settings_sections as $section ) { ?>
                <div id="<?php echo $section['id']; ?>" class="MSettingSection" style="display: none;">
